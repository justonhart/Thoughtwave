...
export class RemoteMiner extends WaveCreep {
    protected run() {
        ...
        if (Memory.rooms[this.memory.room].remoteAssignments[assignedPos.roomName]?.state === RemoteRoomThreatLevel.ENEMY_ATTTACK_CREEPS) {
        ...
        if (this.pos.isEqualTo(assignedPos)) {
            ...
            if (
                Object.values(this.homeroom.memory.remoteAssignments).some(
                    (assignment) => assignment.state === RemoteRoomThreatLevel.ENEMY_ATTTACK_CREEPS
                )
            ) {
                if (this.memory._m) {
                    this.memory._m.repath = 1; // do not create roads
