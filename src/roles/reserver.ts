...
export class Reserver extends WaveCreep {
    protected run() {
        if (Memory.rooms[this.memory.room].remoteAssignments[this.memory.assignment]?.state === RemoteRoomThreatLevel.ENEMY_ATTTACK_CREEPS) {
        ...
        if (!this.memory.destination) {
            ...
            if (
                ...
                this.homeroom.memory.remoteAssignments[this.room.name].controllerState = RemoteRoomReservationStatus.ENEMY;
                ...
                if (!this.room.controller.reservation?.ticksToEnd || this.room.controller.reservation.ticksToEnd < 1000) {
                    this.homeroom.memory.remoteAssignments[this.room.name].controllerState = RemoteRoomReservationStatus.LOW;
                } else if (this.room.controller.reservation.ticksToEnd > 4500) {
                    this.homeroom.memory.remoteAssignments[this.room.name].controllerState = RemoteRoomReservationStatus.STABLE;
                }
