...
export class Miner extends WaveCreep {
    public run() {
        ...
        if (this.pos.isEqualTo(assignedPos)) {
            ...
            this.travelTo(assignedPos, { preferRoadConstruction: true, stayOnPath: true }); // TODO add a reuse? If two miners spawn same time or close to each other then the road does not exist and multiple could be made (so either recalculate path more often or put a timer between remote miner spawns)
